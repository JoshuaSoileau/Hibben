  ...
  listings.forEach(function (listing) {
    templ = $.htmlTemplate(
      "" +
        ...
        '<p class="contact">For more information <a href="mailto:info@hibbenatbellehall.com?subject=Information%20Request%20for%20' +
        swapSpaces(listing.address) +
        '">click here</a> to email an agent today.</p>' +
        "</div>" +
        "</div>" +
        "</div>" +
        "</div>" +
  ...
  Filtering.prototype = {
    ...
    toggleAppliedStatus: function () {
      ...
      if (button.attr("data-filters") == "show-all") {
        ...
        if (!button.hasClass("button--black"))
          this.links
            .filter('[data-filters="show-all"]')
            .removeClass("button--black");
        button.toggleClass("button--black");
        if (!$("#listings .filters .button--black").length) {
          $('#listings .filters button[data-filters="show-all"]').addClass(
            "button--black"
          );
        }
      }
    ...
    clearSiblingFilters: function (filter) {
      var target = $(
          '#listings .filters button[data-filters="' + filter + '"]'
        ),
